...
class NanophotonicCurveFittingModel:
    ...
    def generate_prediction_curves(self):
        """Generate prediction curves for various diameters and refractive indices"""
        
        # Create a grid of diameter values from 10nm to 300nm
        diameters = np.linspace(10, 300, 100)
        
        plt.figure(figsize=(15, 10))
        
        # Plot resonance wavelength vs diameter for different RIs
        plt.subplot(2, 1, 1)
        for ri in self.refractive_indices:
            wavelengths = []
            extinctions = []
            for d in diameters:
                try:
                    wl, ext = self.predict(d, ri)
                    wavelengths.append(wl)
                    extinctions.append(ext)
                except Exception as e:
                    print(f"Error predicting for d={d}, ri={ri}: {e}")
                    wavelengths.append(np.nan)
                    extinctions.append(np.nan)
            
            plt.plot(diameters, wavelengths, '-', label=f'RI = {ri}')
        
        plt.title('Predicted Resonance Wavelength vs Diameter')
        plt.xlabel('Diameter (nm)')
        plt.ylabel('Resonance Wavelength (nm)')
        plt.grid(True)
        plt.legend()
        
        # Plot extinction vs diameter for different RIs
        plt.subplot(2, 1, 2)
        for ri in self.refractive_indices:
            wavelengths = []
            extinctions = []
            ...
            plt.plot(diameters, extinctions, '-', label=f'RI = {ri}')
        
        plt.title('Predicted Extinction vs Diameter')
        plt.xlabel('Diameter (nm)')
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(self.model_dir, 'prediction_curves.png'))
        plt.close()
        
    def optimize_diameter_for_target_wavelength(self, target_wavelength, ri, initial_guess=None, bounds=None):
        """Find optimal diameter for a target resonance wavelength"""
        from scipy.optimize import minimize_scalar
        
        if initial_guess is None:
            # Estimate initial guess based on the observed trend in the image
            # Assuming roughly linear relationship for simplicity of initial guess
            initial_guess = (target_wavelength - 500) / 1.0 + 50  # Simplified linear mapping
        
        if bounds is None:
            bounds = (10, 300)  # Reasonable diameter range in nm
            
        def objective(diameter):
            predicted_wavelength, _ = self.predict(diameter, ri)
            return (predicted_wavelength - target_wavelength) ** 2
        
        result = minimize_scalar(objective, bounds=bounds, method='bounded')
        
        if result.success:
            optimal_diameter = result.x
            predicted_wavelength, predicted_extinction = self.predict(optimal_diameter, ri)
            return {
                'optimal_diameter': optimal_diameter,
                'predicted_wavelength': predicted_wavelength,
                'predicted_extinction': predicted_extinction,
                'wavelength_error': abs(predicted_wavelength - target_wavelength)
            }
        else:
            return None
    
    def optimize_diameter_for_max_extinction(self, wavelength_range, ri, num_samples=100):
        """Find the diameter that maximizes extinction within a wavelength range"""
        diameters = np.linspace(10, 300, num_samples)
        max_extinction = 0
        optimal_diameter = None
        optimal_wavelength = None
        
        for d in diameters:
            wavelength, extinction = self.predict(d, ri)
            
            # Check if wavelength is within desired range
            if wavelength_range[0] <= wavelength <= wavelength_range[1]:
                if extinction > max_extinction:
                    max_extinction = extinction
                    optimal_diameter = d
                    optimal_wavelength = wavelength
        
        if optimal_diameter is not None:
            return {
                'optimal_diameter': optimal_diameter,
                'predicted_wavelength': optimal_wavelength,
                'predicted_extinction': max_extinction
            }
        else:
            return None

    def evaluate_sensitivity(self, diameter, ri_range=(1.0, 1.5), num_samples=20):
        """Evaluate sensitivity of the resonance to refractive index changes"""
        refractive_indices = np.linspace(ri_range[0], ri_range[1], num_samples)
        wavelengths = []
        extinctions = []
        
        for ri in refractive_indices:
            wavelength, extinction = self.predict(diameter, ri)
            wavelengths.append(wavelength)
            extinctions.append(extinction)
        
        # Calculate sensitivity as wavelength shift per unit RI change
        wavelength_slope = np.polyfit(refractive_indices, wavelengths, 1)[0]
        extinction_slope = np.polyfit(refractive_indices, extinctions, 1)[0]
        
        # Plot the sensitivity curves
        plt.figure(figsize=(12, 6))
        
        plt.subplot(1, 2, 1)
        plt.plot(refractive_indices, wavelengths, 'o-')
        plt.title(f'Wavelength Sensitivity (D = {diameter}nm)')
        plt.xlabel('Refractive Index')
        plt.ylabel('Resonance Wavelength (nm)')
        plt.grid(True)
        
        plt.subplot(1, 2, 2)
        plt.plot(refractive_indices, extinctions, 'o-', color='red')
        plt.title(f'Extinction Sensitivity (D = {diameter}nm)')
        plt.xlabel('Refractive Index')
        plt.ylabel('Extinction')
        plt.grid(True)
        
        plt.tight_layout()
        plt.savefig(os.path.join(self.model_dir, f'sensitivity_d{diameter}.png'))
        plt.close()
        
        return {
            'diameter': diameter,
            'wavelength_sensitivity': wavelength_slope,
            'extinction_sensitivity': extinction_slope,
            'refractive_indices': refractive_indices.tolist(),
            'wavelengths': wavelengths,
            'extinctions': extinctions
        }
    
    def interactive_explorer(self):
        """Create an interactive plot to explore the model predictions"""
        import ipywidgets as widgets
        from IPython.display import display
        
        # Create widgets for interactive controls
        diameter_slider = widgets.FloatSlider(
            value=150, min=10, max=300, step=1, description='Diameter (nm)'
        )
        
        ri_dropdown = widgets.Dropdown(
            options=[(f'RI = {ri}', ri) for ri in self.refractive_indices],
        ...
        # Create function to update output based on widget values
        def update_output(diameter, ri):
            with output:
                ...
                try:
                    ...
                    # Create a bar chart showing the predicted values
                    plt.subplot(1, 2, 1)
                    bars = plt.bar(['Resonance Wavelength', 'Extinction'], 
                                   [wavelength, extinction], 
                                   color=['blue', 'red'])
                    
                    # Custom y-scales for each bar
                    ax1 = plt.gca()
                    ax2 = ax1.twinx()
                    ...
                    plt.title(f'Predictions for D={diameter}nm, RI={ri}')
                    
                    # Show position on the curve
                    plt.subplot(1, 2, 2)
                    
                    # Generate curve for current RI
                    diameters = np.linspace(10, 300, 50)
                    wavelengths = []
                    extinctions = []
                    for d in diameters:
                        wl, ext = self.predict(d, ri)
                    ...
                    # Plot curves and current point
                    ax1 = plt.gca()
                    ax2 = ax1.twinx()
                    
                    # Wavelength curve
                    line1, = ax1.plot(diameters, wavelengths, 'b-', label='Wavelength')
                    point1 = ax1.scatter([diameter], [wavelength], color='blue', s=100)
                    ax1.set_ylabel('Wavelength (nm)', color='blue')
                    ax1.tick_params(axis='y', labelcolor='blue')
                    
                    # Extinction curve
                    line2, = ax2.plot(diameters, extinctions, 'r-', label='Extinction')
                    point2 = ax2.scatter([diameter], [extinction], color='red', s=100)
                    ax2.set_ylabel('Extinction', color='red')
                    ax2.tick_params(axis='y', labelcolor='red')
                    
                    plt.xlabel('Diameter (nm)')
                    plt.title(f'Response Curves (RI={ri})')
                    plt.grid(True, alpha=0.3)
                    
                    plt.tight_layout()
                    plt.show()
                    
                    print(f"Predicted resonance wavelength: {wavelength:.2f} nm")
                    print(f"Predicted extinction: {extinction:.2f}")
        ...
        display(widgets.VBox([interactive, output]))

def main():
    # Set the path to your data file
    data_path = "resonancedata.xlsx"
    
    # Initialize the model
    model = NanophotonicCurveFittingModel(data_path, separate_models=True)
    ...
    # Load the data
    try:
        combined_df = model.load_data()
    except Exception as e:
        print(f"Error loading data: {e}")
        return
    
    # Analyze the data
    model.analyze_data(combined_df)
    
    # Check if models exist and load them, otherwise train new models
    try:
        model.load_saved_models()
        if not model.models:  # If no models were loaded
    ...
    model.generate_prediction_curves()
    
    # Example optimization: find diameter for target wavelength
    target_wavelength = 600  # nm
    ri = 1.33  # water
    optimization_result = model.optimize_diameter_for_target_wavelength(target_wavelength, ri)
    
    if optimization_result:
        print("\nOptimization for target wavelength:")
        print(f"Target wavelength: {target_wavelength} nm")
        print(f"Optimal diameter: {optimization_result['optimal_diameter']:.2f} nm")
        print(f"Predicted wavelength: {optimization_result['predicted_wavelength']:.2f} nm")
        print(f"Wavelength error: {optimization_result['wavelength_error']:.2f} nm")
        print(f"Predicted extinction: {optimization_result['predicted_extinction']:.2f}")
    
    # Example: find diameter for maximum extinction in visible range
    visible_range = (400, 700)  # nm
    extinction_result = model.optimize_diameter_for_max_extinction(visible_range, ri=1.33)
    
    if extinction_result:
        print("\nOptimization for maximum extinction in visible range:")
        print(f"Optimal diameter: {extinction_result['optimal_diameter']:.2f} nm")
        print(f"Predicted wavelength: {extinction_result['predicted_wavelength']:.2f} nm")
        print(f"Predicted extinction: {extinction_result['predicted_extinction']:.2f}")
    
    # Example: evaluate sensitivity at a specific diameter
    sensitivity_result = model.evaluate_sensitivity(diameter=150, ri_range=(1.0, 1.5))
    print("\nSensitivity Analysis:")
    print(f"Diameter: {sensitivity_result['diameter']} nm")
    print(f"Wavelength sensitivity: {sensitivity_result['wavelength_sensitivity']:.2f} nm/RIU")
    print(f"Extinction sensitivity: {sensitivity_result['extinction_sensitivity']:.2f} /RIU")
    
    print("\nAll analyses complete. Models and visualizations saved to:", model.model_dir)
    
    # For Jupyter notebook environments, uncomment to enable interactive exploration
    # model.interactive_explorer()
